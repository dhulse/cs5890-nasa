...
scatter = ax.scatter(df['date_int'], df['mining-index'])

for i in df[['object', 'v-relative', 'minimum-AU-distance-normalized']].as_matrix():
    print i

labels = ['<p>Object: {0}</p><p>Velocity: {1}</p><p>Distance AU: {2}</p>'.format(i[0], i[1], i[2]) for i in df[['object', 'v-relative', 'minimum-AU-distance-normalized']].as_matrix()]
